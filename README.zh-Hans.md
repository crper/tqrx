...
[English](./README.md) · [Design](./DESIGN.md) · [Contributing](./CONTRIBUTING.md)

`tqrx` 是一个小而直接的 Go 工具：

- `CLI` 适合快速生成 `PNG` / `SVG`
- `-m` 适合在终端直接打印二维码，无需生成文件
- `TUI` 适合边改内容边看预览再保存
...
- 终端直接打印二维码：`tqrx -m "https://example.com"`
- 保留交互工作台：`tqrx tui`
...
- `pre-push` 会跑 `go test ./...`、`go vet ./...`、`go build ./...`
